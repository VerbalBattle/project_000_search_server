...
var txUrl = "http://52.9.35.57/db/data/node/";
var auth = {
  user: 'neo4j',
  pass: 'E@}Xz5Y7>q5(~pKG',
  ...
  return new Promise(function(resolve, reject) {
        ...
        if (data.length > 0) {
          request.get({
            ...
            if (response.body.exception !== undefined || body.data.text !== username) {
              ...
              request.post(txUrl + nodeId.toString() + '/relationships', {
                  auth: auth,
                  form: {
                    to : txUrl + lastNode.metadata.id.toString(),
                    type : 'KNOWS'
                  }
            ...
            newUser.save(function (err) {
              if (err) {
                ...
                if(lastNode.metadata) {
                  request.post(txUrl + nodeid + '/relationships', {
                      auth: auth,
                      form: {
                        to : txUrl + lastNode.metadata.id,
                        type : 'KNOWS'
                      }
                    }, function (err, res) {
                      if (err) {
    ...
    User.find({text: username}, function(err, data) {
      ...
      if(data.length === 0) {
        ...
        request.put(txUrl + data[0].nodeid + '/properties', {
          auth: auth,
  ...
  return new Promise(function (resolve, reject) {
    ...
    var recurseGetSuggestions = function (nodeID) {
      return new Promise(function (resolveInner, rejectInner) {
        request.get({
          url: txUrl + nodeID.toString() + suffixURL,
          auth: auth,
        }, function (err, httpResponse, body) {
          if (err) {
            rejectInner(err);
...
prefixTree.deleteUser = function(username) {
  User.find({text: username}, function(err, data) {
    if(err) {
      return "error";
    }
    ...
    if(data[0].isUser) {
      // update node to turn isUsername property false
      request.put(txUrl + data[0].nodeid + '/properties', {
        auth: auth,
        form: {isUsername : false}
      }, function (err) {
        if(err) {
          return "error";
        } else {
